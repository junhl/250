...
public class Heap {
    ...
    // Question 3 is here 
    // O(n) is the Big-oh here !
    public Object getMax() throws HeapEmptyException{
      if (isEmpty()){
        throw new HeapEmptyException();
      }
      int now = last;
      //max will hold the currently-largest key value
      // initialized with last, but it is updated in the while loop below
      int max = h[last].key;
      
      //we can use last to search at the lowest level and use decrements (now--) to move level
      while(now!=getParent(last)){ 
        now--;
        // change max value if a new larger one is found
        if(max < h[now].key){
          max = h[now].key;
        }
      }
      return h[max].content;
    }
}